...
def _samples():
    ...
    if SAMPLES_DIR.is_dir():
        for f in sorted(SAMPLES_DIR.glob("*.jpg")):
            ...
            samples.append({"file":f"analyzer/samples/{f.name}","label":SAMPLE_LABELS.get(key,key),"expected":key})
    return samples
...
def _model_cards():
    ...
    for part,task in (("chest","Normal vs. pneumonia"),("bone","Fractured vs. not")):
        m=info.get(part)
        if not m: continue
        cards.append({"name":f"{part.title()} model","task":task,"headline":_pct(m["test_balanced_accuracy"]),"headline_label":"balanced test accuracy","rows":[("Principal components",m["n_components"]),("Variance kept",_pct(m["variance_retained"])),("Classifier","Logistic regression"),("Reasoning-tree fidelity",_pct(m["fidelity"])),("Training images",f"{m['n_images']['normal']} normal · {m['n_images']['abnormal']} abnormal") ]})
    return cards

def index(request):
    return render(request,"analyzer/index.html",{"samples":_samples(),"model_cards":_model_cards(),"max_mb":settings.MAX_UPLOAD_SIZE_BYTES//(1024*1024)})
...
def api_analyze(request):
    ...
    if upload.size>settings.MAX_UPLOAD_SIZE_BYTES:
        return JsonResponse({"status":"error","message":f"That file is larger than {settings.MAX_UPLOAD_SIZE_BYTES//(1024*1024)} MB."},status=400)
    try:
        with Image.open(upload) as probe: probe.verify()
        upload.seek(0); result=analyze_xray(upload)
    except (UnidentifiedImageError,OSError,ValueError):
        return JsonResponse({"status":"error","message":"That file couldn't be read as an image. Try a JPG or PNG."},status=400)
